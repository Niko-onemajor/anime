  ...
  <div class="search-overlay" v-if="showSearchModal" @click.self="closeSearchModal">
    <div class="search-modal">
      <div class="search-modal-header">
        ...
        <input ref="searchInputRef" v-model="userSearchKeyword" class="search-modal-input" placeholder="搜索动漫或用户..." @input="onSearchInput" @keydown.esc="closeSearchModal" />
      ...
      <div class="search-modal-body">
        ...
        <template v-else-if="searchTab === 'anime'">
          ...
          <div v-else class="search-results">
            <div v-for="anime in animeResults" :key="'anime-' + anime.id" class="search-result-item" @click="goToAnimeDetailFromSearch(anime.id)">
  ...
  searchTimer = setTimeout(async () => {
    try {
      const [animeRes, userRes] = await Promise.allSettled([
        axios.post('http://localhost:8080/api/anime/search', { keyword }),
        axios.get('http://localhost:8080/api/user/search', { params: { keyword } })
      ]);
...
const goToAnimeDetailFromSearch = (animeId: number) => {
